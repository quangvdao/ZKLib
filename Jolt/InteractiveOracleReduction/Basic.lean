...
def PartialTranscript.toFull (spec : ProtocolSpec)
  ...
  messages := by
    ...
    simp [Fin.cast_val_eq_self] at f
    rw [Nat.mod_eq_of_lt (a := ↑spec.numRounds) (by simp)] at f
    intro i
    obtain m := f i
    simp [Fin.cast_val_eq_self] at m
    exact m
  challenges := by
    let f := fun j => partialTranscript.challenges j
    simp [Fin.cast_val_eq_self] at f
    rw [Nat.mod_eq_of_lt (a := ↑spec.numRounds) (by simp)] at f
    intro i
    obtain m := f i
