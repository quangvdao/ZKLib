...
import ZKLib.ToMathlib.Tactic.NormNum.PowMod
import Mathlib.Tactic.ReduceModChar.Ext

/-!
# `reduce_mod_char_pow` tactic

Define the `reduce_mod_char_pow` tactic, which traverses expressions looking for numerals `n`,
...
/- Changes compared to Mathlib version -/
...
/- End of changes compared to Mathlib -/
...
-/
syntax (name := reduce_mod_char) "reduce_mod_char_pow" (location)? : tactic
@[inherit_doc reduce_mod_char]
syntax (name := reduce_mod_char!) "reduce_mod_char_pow!" (location)? : tactic

elab_rules : tactic
| `(tactic| reduce_mod_char_pow $[$loc]?) => unsafe do
