...
import ZKLib.RiscV.R0sy.Helper
...
def ByteReader.readBytes [Monad M] [MonadStateOf ByteReader M] [MonadExcept ByteReaderError M] (bytes: Nat) : M (Subarray UInt8)
  := do let self <- get
        if h : bytes >= self.data.size then throw ByteReaderError.OutOfData
        else
          let (result, data) := self.data.split ⟨bytes, by omega⟩
          set { self with data }
          pure result
...
instance [Monad M] [MonadStateOf ByteReader M] [MonadExcept ByteReaderError M] : MonadByteReader M where
  ...
  onSubarray {X} (start stop: Nat) (f: M X)
    := do let self <- get
          let sub: ByteReader := { data := self.data.array.toSubarray start stop }
          set sub
          let x <- f
          set self
          pure x
  getSubarray (start stop: Nat)
    := do let self <- get
          pure (self.data.array.toSubarray start stop)
